...
namespace NiuMa
{
	HongZhongMahjongRoom::HongZhongMahjongRoom(const std::string& venueId, const std::string& number, int level, const std::string& ruleConfig)
		: MahjongRoom(std::make_shared<MahjongRule>(), venueId, static_cast<int>(GameType::HongZhongMahjong))
		, _number(number)
		, _level(level)
		...
		for (int i = 0; i < getMaxPlayerNums(); i++) {
			...
			while (it != lstChapters.end()) {
				...
				if (chapterType == MahjongChapter::Type::ZhiGang) {
					...
					avatar2 = dynamic_cast<HongZhongMahjongAvatar*>(getAvatar(seat).get());
					if (avatar2 != nullptr) {
