...
namespace NiuMa
{
	TaoJiangMahjongRecordTask::TaoJiangMahjongRecordTask()
		: _roundNo(0)
		, _banker(0)
	{
		for (int i = 0; i < 4; i++) {
	...
	MysqlQueryTask::QueryType TaoJiangMahjongRecordTask::buildQuery(std::string& sql) {
		std::stringstream ss;
		ss << "insert into `game_taojiang_mahjong_record`(`venue_id`, `round_no`, `banker`, "
